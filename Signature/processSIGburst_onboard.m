function eps = processSIGburst_onboard(wraw,cs,dz,bz,neoflp,rmin,rmax,nzfit,avgtype,fittype)
...
cs = nanmean(cs,'omitnan');
Vr = cs.^2./(4*F0*L);% m/s
nfilt = round(1/dz);% 1 m

% Identify Spikes
wfilt = movmedian(wraw,nfilt,'omitnan');% was medfilt1
ispike = abs(wraw - wfilt) > Vr/2;
