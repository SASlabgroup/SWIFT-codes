...
% K.Zeiden June 2024
% M.LeClair April 2026 - parallel & fix
...
for iburst = 1:nburst
    ...
    elseif any(strcmp({vars.name},'Data')) && any(strcmp({vars.name},'Config'))
        S = load([bfiles(iburst).folder slash bfiles(iburst).name]);
        Config = S.Config;
        Data = S.Data;
    else
    ...
    if isfield(Data,'BurstHR_MatlabTimeStamp')
        ...
        % 5-beam HR
        elseif Config.bursthr_enable5 && Config.bursthr_enable
    ...
    %% Save
    outname = [bfiles(iburst).folder slash bfiles(iburst).name(1:end-4) '_reformat.mat'];
    if exist('burst','var') && exist('echo','var')
        save(outname, 'burst','avg','echo')
    elseif exist('burst','var')
        save(outname, 'burst','avg')
    else
        save(outname, 'avg');
    end
