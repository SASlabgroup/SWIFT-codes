function [lat, lon] = xytoll_RIVET(x, y)
...
if ( (n ~= n2) | (m~=m2) ),
    disp('Bad sizes of x and y to xytoll_RIVET.m')
    return
end;
...
if (isrow(x)),
    ...
    if (iscolumn(x)),
        ...
        disp('Need either column or row vector for x & y input to xytoll_RIVET.m');
        return;
    end;
end;
...
%% Rotate x,y data by -theta radians to N,E orientation:
