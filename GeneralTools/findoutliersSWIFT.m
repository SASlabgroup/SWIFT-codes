function [nonoutliersSWIFT,outliersSWIFT,booloutSWIFT] = findoutliersSWIFT(SWIFT, method, pct,varargin)
...
for q = 1:length(fields)
    if string(class([SWIFT(1).(fields{q})])) == "double" & string(fields{q}) ~="time"
        ...
        for k = 1:length(SWIFT)
            ...
            nonoutliersSWIFT(k).time = SWIFT(k).time;
            outliersSWIFT(k).time = SWIFT(k).time;
            if booloutSWIFTstruct.(fields{q})(k) == 1;
                outliersSWIFT(k).(fields{q}) = SWIFT(k).(fields{q});
                nonoutliersSWIFT(k).(fields{q}) = nan(size(SWIFT(k).(fields{q})));
            else
                nonoutliersSWIFT(k).(fields{q}) = SWIFT(k).(fields{q});
                outliersSWIFT(k).(fields{q}) = nan(size(SWIFT(k).(fields{q})));
            end
        end
