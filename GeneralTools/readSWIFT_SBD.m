...
%         imwrite(rotatedimage, [fname '.jpg'],'JPEG')
    ...
    elseif type == 54 & size >= 303 % microSWIFT light sensor, size either 303 bytes (legacy) or 329 (Apr 2026)
        ...
        SWIFT.acc_min(1)         = half.typecast(fread(fid,1,'*uint16')).double;  % min accelerations, x (units?) 
        SWIFT.acc_max(1)         = half.typecast(fread(fid,1,'*uint16')).double;  % max accelerations, x (units?) 
        SWIFT.acc_mean(1)        = half.typecast(fread(fid,1,'*uint16')).double;  % mean accelerations, x (units?) 
        SWIFT.acc_min(2)         = half.typecast(fread(fid,1,'*uint16')).double;  % min accelerations, y (units?) 
        SWIFT.acc_max(2)         = half.typecast(fread(fid,1,'*uint16')).double;  % max accelerations, y (units?) 
        SWIFT.acc_mean(2)        = half.typecast(fread(fid,1,'*uint16')).double;  % mean accelerations, y (units?) 
        SWIFT.acc_min(3)         = half.typecast(fread(fid,1,'*uint16')).double;  % min accelerations, z (units?) 
        SWIFT.acc_max(3)         = half.typecast(fread(fid,1,'*uint16')).double;  % max accelerations, z (units?) 
        SWIFT.acc_mean(3)        = half.typecast(fread(fid,1,'*uint16')).double;  % mean accelerations, z (units?) 
