...
%             6/2022 add direction at energy period (in addition to peak)
...
for si = 1:length([SWIFT1.time]),
    ...
    % calculate energy period and dir (for later comparison)
    if isfield(SWIFT1(1),'wavespectra'),
        ...
        [fdiff feindex] = min( abs(SWIFT1(si).wavespectra.freq - 1./SWIFT1(si).energyperiod ) );
        dir1 = atan2(SWIFT1(si).wavespectra.b1,SWIFT1(si).wavespectra.a1) ;
    ...
    if tdiff(si) < tlimit & rdiff(si) < rlimit,
        ...
        if isfield(SWIFT2(1),'wavespectra'),
            SWIFT2(match).energyperiod = nansum(SWIFT2(match).wavespectra.energy ) ./ nansum(SWIFT2(match).wavespectra.energy ...
                .* SWIFT2(match).wavespectra.freq);
            [fdiff feindex] = min( abs(SWIFT2(match).wavespectra.freq - 1./SWIFT2(match).energyperiod ) );
            dir1 = atan2(SWIFT2(match).wavespectra.b1,SWIFT2(match).wavespectra.a1) ;
            dir1 = - 180 ./ 3.14 * dir1;  % switch from rad to deg, and CCW to CW (negate)
            dir1 = dir1 + 90;  % rotate from eastward = 0 to northward  = 0
            dir1( dir1 < 0 ) = dir1( dir1 < 0 ) + 360;  % take NW quadrant from negative to 270-360 range
            westdirs = dir1 > 180;
