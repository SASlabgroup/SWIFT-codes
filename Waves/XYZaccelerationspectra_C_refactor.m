...
% Since fftX[0] is the constant offset, our frequency band windows start at the 2nd window_point, so need strict inequality.
...
for win_idx=1:num_windows
    ...
    for ii = 1 : 1 : nfbands % Iterate over merged frequency bins
        ...
        for jj=1:merge % Iterate over frequency in each bin
            magx = real(fft_x(idx0 + jj) * conj(fft_x(idx0 + jj)));
            magy = real(fft_y(idx0 + jj) * conj(fft_y(idx0 + jj)));
            magz = real(fft_z(idx0 + jj) * conj(fft_z(idx0 + jj)));
            XX_bands(ii) = XX_bands(ii) + magx / denom;
            YY_bands(ii) = YY_bands(ii) + magy / denom;
            ZZ_bands(ii) = ZZ_bands(ii) + magz / denom;
        end
    end
end
